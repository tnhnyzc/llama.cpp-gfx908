...
ggml_tensor * llama_model_qwen35::graph::build_layer_attn_linear(
    ...
    //
    // Gated so one binary holds both paths: the effect is smaller than
    // run-to-run spread, so the A/B must not also vary the build.
    // LLAMA_QWEN35_FUSED_QK_L2NORM=0 restores the original two calls.
    static const bool fused_qk_l2norm = []() {
        const char * e = getenv("LLAMA_QWEN35_FUSED_QK_L2NORM");
