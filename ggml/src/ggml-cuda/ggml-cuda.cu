...
// Experimental: extend gate fusion to ncols_dst == 4 on CDNA1 for IQ4_NL only.
// Without it, B=4 loses the fusion that B=1 and B=2 get, and the gate becomes a
// separate dispatch -- the dominant projection goes from ~64 to ~128 dispatches
// per decode step, which is exactly the measured B=2..4 batching valley.
static bool ggml_cuda_should_fuse_mul_mat_vec_q_gfx908_m4(const ggml_tensor * tensor) {
    static const bool disable_gfx908_m4_gate_fusion =
        getenv("GGML_HIP_DISABLE_GFX908_M4_GATE_FUSION") != nullptr;
    if (disable_gfx908_m4_gate_fusion) {
        return false;
    }
    ...
    const int cc = ggml_cuda_info().devices[ggml_cuda_get_device()].cc;
    return tensor->op == GGML_OP_MUL_MAT && tensor->ne[1] == 4 &&
           cc == GGML_CUDA_CC_CDNA1 && src0->type == GGML_TYPE_IQ4_NL &&
           !bad_padding_clear && src1->type == GGML_TYPE_F32 && tensor->type == GGML_TYPE_F32 &&
           src1->ne[1] <= MMVQ_MAX_BATCH_SIZE;
}
    ...
    for (ggml_op op : { GGML_OP_MUL_MAT, GGML_OP_MUL_MAT_ID }) {
        ...
        if (ggml_cuda_can_fuse(cgraph, i, { op, bias_op, op, bias_op, GGML_OP_GLU }, {})) {
            ...
            const bool use_quant_m2_fusion =
                ...
                ggml_cuda_should_fuse_mul_mat_vec_q_gfx908_m4(up);
