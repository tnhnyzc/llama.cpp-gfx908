...
static __global__ void mul_mat_vec_f(
        ...
        const int ids_stride, const int expert_base, const int expert_count) {
    ...
    if constexpr (is_multi_token_id) {
        ...
        expert_id  = ids[channel_dst + token_idx * ids_stride];
        channel_x  = expert_id - expert_base;
        channel_y  = fastmodulo(channel_dst, nchannels_y);
        sample_dst = 0;
    } else {
        token_idx  = ids ? blockIdx.z                                          : 0;
        expert_id  = ids ? ids[blockIdx.y + token_idx * ids_stride]            : 0;
        channel_x  = ids ? expert_id - expert_base                             : fastdiv((uint32_t) channel_dst, channel_ratio);
    ...
    if (ids && (channel_x < 0 || channel_x >= expert_count)) {
        if (tid == 0) {
#pragma unroll
            for (int j = 0; j < ncols_dst; ++j) {
                dst[j*stride_col_dst + row] = 0.0f;
            }
        }
        return;
    }
...
static void mul_mat_vec_f_switch_fusion(
        ...
        const uint3 channel_ratio, const int stride_channel_x, const int stride_channel_y, const int stride_channel_dst,
        const uint3 sample_ratio, const int stride_sample_x, const int stride_sample_y, const int stride_sample_dst,
        const dim3 & block_dims, const dim3 & block_nums, const int nbytes_shared, const int ids_stride,
        const int expert_base, const int expert_count, const cudaStream_t stream) {
    ...
    if constexpr (ncols_dst == 1) {
        if (has_fusion) {
            ggml_cuda_kernel_launch(mul_mat_vec_f<T, type_acc, ncols_dst, block_size, true, is_multi_token_id>, launch_params,
                ...
                sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride, expert_base, expert_count);
            return;
       }
    }
...
void launch_mul_mat_vec_f_cuda(
        ...
        const int64_t nsamples_or_ntokens, const int64_t ids_stride,
        const int expert_base, const int expert_count, cudaStream_t stream) {
        ...
        case   32: {
            mul_mat_vec_f_switch_fusion<T, type_acc, ncols_dst, 32, is_multi_token_id>
                (x, y, ids, fusion, dst, ncols/2, nchannels_y_fd, stride_row, stride_col_y/2, stride_col_dst,
                 ...
                 sample_ratio_fd, stride_sample_x, stride_sample_y, stride_sample_dst, block_dims, block_nums, nbytes_shared, ids_stride, expert_base, expert_count, stream);
        } break;
        case   64: {
...
static void mul_mat_vec_f_cuda_switch_ncols_dst(
        ...
        const int64_t stride_channel_x, const int64_t stride_channel_y, const int64_t stride_channel_dst, const int64_t nsamples_x,
        const int64_t nsamples_dst, const int64_t stride_sample_x, const int64_t stride_sample_y, const int64_t stride_sample_dst,
        const int64_t ids_stride, const int expert_base, const int expert_count, cudaStream_t stream) {
    ...
    if (has_ids && ncols_dst > 1) {
        ...
        launch_mul_mat_vec_f_cuda<T, type_acc, c_ncols_dst, true>
            (x, y, ids, fusion, dst, ncols, nrows, stride_row, stride_col_y, stride_col_dst,
             ...
             ncols_dst, ids_stride, expert_base, expert_count, stream);
        return;
    }
        ...
        case 1:
            launch_mul_mat_vec_f_cuda<T, type_acc, 1>
                (x, y, ids, fusion, dst, ncols, nrows, stride_row, stride_col_y, stride_col_dst,
                 nchannels_x, nchannels_y, nchannels_dst, stride_channel_x, stride_channel_y,
                 stride_channel_dst, nsamples_x, nsamples_dst, stride_sample_x, stride_sample_y, stride_sample_dst,
                 nsamples_dst, ids_stride, expert_base, expert_count, stream);
            break;
        case 2:
...
static void mul_mat_vec_f_cuda(
        ...
        const int64_t stride_channel_x, const int64_t stride_channel_y, const int64_t stride_channel_dst, const int64_t nsamples_x,
        const int64_t nsamples_dst, const int64_t stride_sample_x, const int64_t stride_sample_y, const int64_t stride_sample_dst,
        const int64_t ids_stride, enum ggml_prec prec,
        const int expert_base, const int expert_count, cudaStream_t stream) {
    ...
    if constexpr(std::is_same_v<T, half>) {
        if (prec == GGML_PREC_DEFAULT) {
            mul_mat_vec_f_cuda_switch_ncols_dst<T, half>
                ...
                nchannels_x, nchannels_y, nchannels_dst, stride_channel_x, stride_channel_y,
                stride_channel_dst, nsamples_x, nsamples_dst, stride_sample_x, stride_sample_y, stride_sample_dst,
                ids_stride, expert_base, expert_count, stream);
            return;
        }
    }
...
void ggml_cuda_mul_mat_vec_f(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * ids,
    ...
    GGML_ASSERT(!ids || (mmid != nullptr && mmid->op == GGML_OP_MUL_MAT_ID));
    ...
    const int local_expert_count = ids ? ggml_get_op_params_i32(mmid, 3) : 0;
    const int expert_base  = local_expert_count != 0 ? ggml_get_op_params_i32(mmid, 2) : 0;
    const int expert_count = local_expert_count != 0 ? local_expert_count : ne02;
    GGML_ASSERT(local_expert_count == 0 || local_expert_count == ne02);
    ...
    switch (src0->type) {
        case GGML_TYPE_F32: {
            ...
            mul_mat_vec_f_cuda(src0_d, src1_d, ids_d, fusion_local, dst_d, ne00, ne01, ncols_dst, s01, stride_col_y, stride_col_dst,
                ...
                ne03,              ne3,           s03, s13,              s3,
                ids_stride, prec, expert_base, expert_count, ctx.stream());
        } break;
        case GGML_TYPE_F16: {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32: {
            ...
            mul_mat_vec_f_cuda(src0_d, src1_ddf_i, nullptr, empty, dst_dd_i, ne00, row_diff, src1_ncols, stride_row, stride_col_y, stride_col_dst,
                ...
                nsamples_x, nsamples_dst, stride_sample_x, stride_sample_y, stride_sample_dst, 0, prec, 0, 1, stream);
        } break;
        case GGML_TYPE_F16: {
