...
static void mul_mat_vec_q_switch_fusion(
    ...
    if constexpr (c_ncols_dst == 4 && type == GGML_TYPE_IQ4_NL) {
        if (has_fusion) {
            const ggml_cuda_kernel_launch_params launch_params = ggml_cuda_kernel_launch_params(block_nums, block_dims, nbytes_shared, stream);
            ggml_cuda_kernel_launch(mul_mat_vec_q<type, c_ncols_dst, true, small_k>, launch_params,
                 vx, vy, ids, fusion, dst, ncols_x, nchannels_y, stride_row_x, stride_col_y, stride_col_dst,
    ...
    GGML_ASSERT(!has_fusion && "fusion only supported for ncols_dst=1 (and the gfx908 m2/m4 paths)");
...
void ggml_cuda_mul_mat_vec_q(
    ...
    if (fusion) {
        ...
        const bool gfx908_m2_gate_fusion =
            ...
            fusion->x_scale == nullptr && fusion->gate_scale == nullptr;
        // Experimental ncols_dst == 4 gate fusion, same narrow conditions as m2.
        const bool gfx908_m4_gate_fusion =
            !ids && cc == GGML_CUDA_CC_CDNA1 && dst->ne[1] == 4 &&
            src0->type == GGML_TYPE_IQ4_NL &&
            fusion->gate != nullptr && fusion->x_bias == nullptr && fusion->gate_bias == nullptr &&
            fusion->x_scale == nullptr && fusion->gate_scale == nullptr;
        GGML_ASSERT(ids || dst->ne[1] == 1 || gfx908_m2_gate_fusion || gfx908_m4_gate_fusion);
