...
void ggml_cuda_mul_mat_vec_q(
    ...
    static const bool mixed_q8_no_reuse = [] {
        const char * env = getenv("GGML_HIP_MIXED_NORM_Q8_NO_REUSE_GFX908");
        return env != nullptr && atoi(env) != 0;
    }();
    const auto mixed_q8 = ctx.mixed_norm_q8_1_inputs.find(src1);
    if (!mixed_q8_no_reuse && mixed_q8 != ctx.mixed_norm_q8_1_inputs.end()) {
        src1_q8_1_d = (char *) mixed_q8->second;
    } else {
