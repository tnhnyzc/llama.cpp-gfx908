...
template <
    ...
    int c_rows_override = 0, bool c_gfx908_minwaves6 = false,
    bool c_iq4_shared_unpack = false>
__launch_bounds__(
    calc_nwarps(type, ncols_dst, get_device_table_id())*ggml_cuda_get_physical_warp_size(),
    c_gfx908_minwaves6 ? 6 : 1)
...
#pragma unroll
        ...
        } else if constexpr (type == GGML_TYPE_IQ4_NL && ncols_dst >= 3 &&
                             ncols_dst <= 4 && c_iq4_shared_unpack) {
#pragma unroll
            for (int i = 0; i < rows_per_cuda_block; ++i) {
...
// Measurement-gated gfx908 experiment for Qwen3.6's IQ4_NL FFN-down projection.
// The long-K N=3 kernel benefits from a six-wave register budget while the
// companion short-K gate/up projection regresses under the same constraint.
// Keep the variant opt-in and exact-shape guarded until end-to-end validation.
static bool iq4_nl_n3_long_k_minwaves6_env() {
    static const bool enabled = []() {
        const char * env = std::getenv("GGML_HIP_IQ4_NL_N3_LB6_GFX908");
...
template<
    ...
    bool c_gfx908_minwaves6 = false, bool c_iq4_shared_unpack = false>
static void mul_mat_vec_q_switch_fusion(
    ...
    if constexpr (c_ncols_dst == 1) {
            ...
            if (rows_override > 1) {
                ...
                if (has_fusion) {
                    switch (rows_override) {
                        case 2:
                            ggml_cuda_kernel_launch(mul_mat_vec_q<type, c_ncols_dst, true, small_k, 2, c_gfx908_minwaves6, c_iq4_shared_unpack>, launch_params,
                                 vx, vy, ids, fusion, dst, ncols_x, nchannels_y, stride_row_x, stride_col_y, stride_col_dst,
                                 channel_ratio, stride_channel_x, stride_channel_y, stride_channel_dst,
                                 sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride);
                            return;
                        case 4:
                            ggml_cuda_kernel_launch(mul_mat_vec_q<type, c_ncols_dst, true, small_k, 4, c_gfx908_minwaves6, c_iq4_shared_unpack>, launch_params,
                                 vx, vy, ids, fusion, dst, ncols_x, nchannels_y, stride_row_x, stride_col_y, stride_col_dst,
                                 channel_ratio, stride_channel_x, stride_channel_y, stride_channel_dst,
                                 sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride);
                            return;
                        case 8:
                            ggml_cuda_kernel_launch(mul_mat_vec_q<type, c_ncols_dst, true, small_k, 8, c_gfx908_minwaves6, c_iq4_shared_unpack>, launch_params,
                                 vx, vy, ids, fusion, dst, ncols_x, nchannels_y, stride_row_x, stride_col_y, stride_col_dst,
                                 channel_ratio, stride_channel_x, stride_channel_y, stride_channel_dst,
                                 sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride);
                            return;
                    ...
                    switch (rows_override) {
                        case 2:
                            ggml_cuda_kernel_launch(mul_mat_vec_q<type, c_ncols_dst, false, small_k, 2, c_gfx908_minwaves6, c_iq4_shared_unpack>, launch_params,
                                 vx, vy, ids, fusion, dst, ncols_x, nchannels_y, stride_row_x, stride_col_y, stride_col_dst,
                                 channel_ratio, stride_channel_x, stride_channel_y, stride_channel_dst,
                                 sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride);
                            return;
                        case 4:
                            ggml_cuda_kernel_launch(mul_mat_vec_q<type, c_ncols_dst, false, small_k, 4, c_gfx908_minwaves6, c_iq4_shared_unpack>, launch_params,
                                 vx, vy, ids, fusion, dst, ncols_x, nchannels_y, stride_row_x, stride_col_y, stride_col_dst,
                                 channel_ratio, stride_channel_x, stride_channel_y, stride_channel_dst,
                                 sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride);
                            return;
                        case 8:
                            ggml_cuda_kernel_launch(mul_mat_vec_q<type, c_ncols_dst, false, small_k, 8, c_gfx908_minwaves6, c_iq4_shared_unpack>, launch_params,
                                 vx, vy, ids, fusion, dst, ncols_x, nchannels_y, stride_row_x, stride_col_y, stride_col_dst,
                                 channel_ratio, stride_channel_x, stride_channel_y, stride_channel_dst,
                                 sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride);
                            return;
        ...
        if (has_fusion) {
            const ggml_cuda_kernel_launch_params launch_params = ggml_cuda_kernel_launch_params(block_nums, block_dims, nbytes_shared, stream);
            ggml_cuda_kernel_launch(mul_mat_vec_q<type, c_ncols_dst, true, small_k, 0, c_gfx908_minwaves6, c_iq4_shared_unpack>, launch_params,
                 vx, vy, ids, fusion, dst, ncols_x, nchannels_y, stride_row_x, stride_col_y, stride_col_dst,
                 channel_ratio, stride_channel_x, stride_channel_y, stride_channel_dst,
                 sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride);
            return;
    ...
    const ggml_cuda_kernel_launch_params launch_params = ggml_cuda_kernel_launch_params(block_nums, block_dims, nbytes_shared, stream);
    ggml_cuda_kernel_launch(mul_mat_vec_q<type, c_ncols_dst, false, small_k, 0, c_gfx908_minwaves6, c_iq4_shared_unpack>, launch_params,
        vx, vy, ids, fusion, dst, ncols_x, nchannels_y, stride_row_x, stride_col_y, stride_col_dst,
        channel_ratio, stride_channel_x, stride_channel_y, stride_channel_dst,
        sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride);
}
