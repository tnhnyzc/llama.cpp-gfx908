...
static void launch_mul_mat_q(ggml_backend_cuda_context & ctx, const mmq_args & args, cudaStream_t stream) {
    ...
    // Isolated gfx908/IQ4_NL experiment: when ordinary tiling already fills
    // the device, avoid Stream-K's temporary buffer and fixup dispatch.
    const bool use_tiling = (GGML_CUDA_CC_IS_NVIDIA(cc) ||
        (GGML_CUDA_CC_IS_CDNA1(cc) && type == GGML_TYPE_IQ4_NL)) && tiles_efficiency_percent >= 90;
